...
0. **Spindle/Laser Enable (M3/M4):** turns the output on with M3 or M4, and off with M5
1. **Mist Enable (M7):** turns on/off with M7/M9
2. **Flood Enable (M8):** turns on/off with M8/M9
3. **M62-M65 Only:** always available as a backup to control each port with a unique M command:
